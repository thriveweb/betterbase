...
function get_pagination($range = 2) {
    ...
    if ($max_page > 1) {
        ...
        if ($paged > ($range + 1)) {
            echo '<a href="' . get_pagenum_link(1) . '" class="' . (1 == $paged ? 'is-current' : '') . '">1</a>';
            if ($paged > ($range + 2)) {
                echo '<span>···</span>';
            }
        }
        ...
        for ($i = max(1, $paged - $range); $i <= min($max_page, $paged + $range); $i++) {
            echo '<a href="' . get_pagenum_link($i) . '" class="' . ($i == $paged ? 'is-current' : '') . '">' . $i . '</a>';
        }
        ...
        if ($paged < ($max_page - $range)) {
            ...
            echo '<a href="' . get_pagenum_link($max_page) . '" class="' . ($max_page == $paged ? 'is-current' : '') . '">' . $max_page . '</a>';
        }
